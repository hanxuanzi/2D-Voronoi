...
#include "../VCalculate/VoroPProperties.h"
...
class container : public container_base, public radius_mono {
	public:
		...
		{
			Point<> p1, p2;
			Vedge(Point<> p1, Point<> p2)
			{
				this->p1 = p1;
				this->p2 = p2;
			}
			int contourpoint_id;
			bool operator==(const Vedge& ps)
			{
				if (this->p1 == ps.p1 && this->p2 == ps.p2)
				{
					return true;
				}
				return false;
		...
		std::vector<Vedge> Vorocelledge;
		std::vector<std::vector<Vedge>>VoroE;
		//重载
		template<class c_loop>
		void draw_cells_pov(c_loop& vl, std::vector<Point<>>& edgepoint1, std::vector<Point<>>& edgepoint2, std::vector<std::vector<Vedge>>& Voroedge) {
			voronoicell c; double* pp;
			Voroedge.clear();
			if (vl.start()) do if (compute_cell(c, vl)) {
				Vorocelledge.clear();
				...
				edgepoint2.clear();
				pp = p[vl.ijk] + ps * vl.q;
				//Vorosite.push_back(vec(pp[0], pp[1], pp[2]));
				c.draw_pov(*pp, pp[1], pp[2], edgepoint1, edgepoint2);
				for (int id_edge = 0; id_edge < edgepoint1.size(); id_edge++)
				{
					Vorocelledge.push_back(Vedge(edgepoint1[id_edge], edgepoint2[id_edge]));
				}
				Voroedge.push_back(Vorocelledge);
			} while (vl.inc());
		}
		...
		//重载
		inline void draw_cells_pov(std::vector<Point<>>& edgepoint1, std::vector<Point<>>& edgepoint2, std::vector<std::vector<Vedge>>& Voroedge) {
			c_loop_all vl(*this);
			Voroedge.clear();
			draw_cells_pov(vl, edgepoint1, edgepoint2, Voroedge);
		}
		/** Computes all Voronoi cells and saves the output in POV-Ray
		 * format.
		 * \param[in] filename the name of the file to write to. */
		...
		//重载
		template<class c_loop>
		void print_custom(c_loop& vl, const char* format, VoroPProperties* VoroPProperties) {
			int ijk, q; double* pp;
			if (contains_neighbor(format)) {
				voronoicell_neighbor c;
				if (vl.start()) do if (compute_cell(c, vl)) {
					ijk = vl.ijk; q = vl.q; pp = p[ijk] + ps * q;
					c.output_custom(format, id[ijk][q], *pp, pp[1], pp[2], default_radius, VoroPProperties);
				} while (vl.inc());
			}
			else {
				voronoicell c;
		...
		void print_custom(const char* format, VoroPProperties* VoroPProperties);//重载
