...
void voronoicell_base::draw_pov(double x, double y, double z, std::vector<Point<>>&edgepoint1, std::vector<Point<>>&edgepoint2) {
	int i, j, k; double *ptsp = pts, *pt2;
	for (i = 0; i < p; i++, ptsp += 3) {
		...
		for (j = 0; j < nu[i]; j++) {
			...
			if (k < i) {
				pt2 = pts + 3 * k;
				edgepoint1.emplace_back(Point<>(x + *ptsp*0.5, y + ptsp[1] * 0.5, z + ptsp[2] * 0.5));
				edgepoint2.emplace_back(Point<>(x + *pt2*0.5, y + 0.5*pt2[1], z + 0.5*pt2[2]));
			}
		}
	}
}
...
//重载
void voronoicell_base::output_vertices(int i, double x, double y, double z, VoroPProperties* VoroPProperties) {
	//int j = VoroPProperties->vertices.size();
	if (p > 0) {
		vector<Point<>> tmp;
		vector<int> CellIndex;
		//fprintf(fp, "(%g,%g,%g)", x + *pts*0.5, y + pts[1] * 0.5, z + pts[2] * 0.5);
		
		Point<> PP(x + *pts * 0.5, y + pts[1] * 0.5, z + pts[2] * 0.5 );
		
		tmp.emplace_back(PP);
		auto ite = VoroPProperties->all_vertices.find(PP);
		if (ite == VoroPProperties->all_vertices.end())
		{
			int last_index = VoroPProperties->all_vertices.size();
			VoroPProperties->all_vertices.insert(pair<Point<>, int>(PP, last_index));
			VoroPProperties->AllV.emplace_back(PP);
			CellIndex.emplace_back(last_index);
		}
		else
		{
			CellIndex.emplace_back(ite->second);
		}

		//VoroPProperties->all_vertices.push_back(vec(x + *pts * 0.5, y + pts[1] * 0.5, z + pts[2] * 0.5));
		for (double* ptsp = pts + 3; ptsp < pts + 3 * p; ptsp += 3)//fprintf(fp, " (%g,%g,%g)", x + *ptsp*0.5, y + ptsp[1] * 0.5, z + ptsp[2] * 0.5);
		{
			Point<> PPP ( x + *ptsp * 0.5, y + ptsp[1] * 0.5, z + ptsp[2] * 0.5 );
			tmp.push_back(PPP);

			auto itP = VoroPProperties->all_vertices.find(PPP);
			if (itP == VoroPProperties->all_vertices.end())
			{
				int last_index = VoroPProperties->all_vertices.size();
				VoroPProperties->all_vertices.insert(pair<Point<>, int>(PPP, last_index));
				VoroPProperties->AllV.emplace_back(PPP);
				CellIndex.emplace_back(last_index);
			}
			else
			{
				CellIndex.emplace_back(itP->second);
			}

			//VoroPProperties->all_vertices.push_back(vec(x + *ptsp * 0.5, y + ptsp[1] * 0.5, z + ptsp[2] * 0.5));
		}
		VoroPProperties->vertices.emplace_back(pair<int, vector<Point<>>>(i, tmp));//第i个胞元 tmp角点坐标
		VoroPProperties->VerticesIndex.emplace_back(CellIndex);//每个胞元的角点索引存储
	}
	//j++;
}
...
//重载
//void voronoicell_base::output_custom(const char* format, int i, double x, double y, double z, double r, VoroPProperties* VoroPProperties) {
//	char* fmp = (const_cast<char*>(format));
//	std::vector<int> vi;
//	std::vector<double> vd;
//	while (*fmp != 0) {
...
//			fmp++;
//			switch (*fmp) {
//				//%i %q %s %F %a %f %t %l %n 站点id 站点坐标 %i,%q,%w,%P,%s,%t,%C
//				// Particle-related output  站点输出：
//			//case 'i': fprintf(fp, "%d", i); break;
//			case 'i': VoroPProperties->id_sites.push_back(i); break;//输出站点id
//			//case 'x': fprintf(fp, "%g", x); break;
//			//case 'y': fprintf(fp, "%g", y); break;
//			//case 'z': fprintf(fp, "%g", z); break;
//			case 'q': VoroPProperties->sites.push_back(vec(x, y, z)); break;//输出站点坐标
//			//case 'q': fprintf(fp, "%g %g %g", x, y, z); break;
//			//case 'r': fprintf(fp, "%g", r); break;
//
//				// Vertex-related output  角点输出：
//			//case 'w': fprintf(fp, "%d", p); break;//角点的个数
//			case 'w': VoroPProperties->num_vertices.push_back(pair<int, int>(i, p)); break;//角点的个数
//			//case 'p': output_vertices(fp); break;
//			//case 'P': output_vertices(x, y, z, fp); break;//角点的坐标
//			case 'P': output_vertices(i, x, y, z, VoroPProperties); break;//角点的坐标
//			//case 'o': output_vertex_orders(fp); break;
//			//case 'm': fprintf(fp, "%g", 0.25 * max_radius_squared()); break;
//
//				// Edge-related output
...
//
//				// Face-related output
//			//case 's': fprintf(fp, "%d", number_of_faces()); break;//存放voro单元面数
//			case 's': VoroPProperties->num_faces.push_back(pair<int, int>(i, number_of_faces())); break;//存放voro单元面数
//			//case 'F': fprintf(fp, "%g", surface_area()); break;//存放voro单元的总面积
//			case 'F': VoroPProperties->all_face_area.push_back(surface_area()); break;//存放voro面的总面积
//			/*case 'A': {
//				face_freq_table(vi);
//				voro_print_vector(vi, fp);
//			} break;*/
...
//					if (vi[i] >= 0)
//					{
//						face_id.push_back(vi[i]);
//					}
//				}
//				VoroPProperties->face_id.push_back(face_id);
//				break;
//			}
//			//case 'f': face_areas(vd); voro_print_vector(vd, fp); break;
//			case 'f'://计算单个面片面积
//			{
//				face_areas(vd);
//				VoroPProperties->face_area.push_back(vd);//
//				break;
//			}
//			case 't': //胞元的每个面 用面片所在角点表示
//			{
//				face_vertices(vi);
//				//voro_print_face_vertices(vi, fp);
//				voro_print_face_vertices(i, vi, VoroPProperties);
//			}break;
//			case 'l'://存放voro单元各个面的法矢
//			{
//				normals(vd);
//				//voro_print_positions(vd, fp);
//				voro_print_positions(vd, VoroPProperties);
//				break;
//			}
//			//case 'n': neighbors(vi);
//			//	voro_print_vector(vi, fp);
...
//				centroid(cx, cy, cz);
//				//fprintf(fp, "%g %g %g", x + cx, y + cy, z + cz);
//				VoroPProperties->vorocellcenter.push_back(vec(x + cx, y + cy, z + cz));
//			} break;
//
//			// End-of-string reached
//			case 0: fmp--; break;
...
void voronoicell_base::output_custom(const char* format, int i, double x, double y, double z, double r, VoroPProperties* VoroPProperties) {
	...
	//VoroPProperties->id_sites.emplace_back(i);//输出站点id
	//VoroPProperties->sites.emplace_back(vec(x, y, z));//输出站点坐标
	VoroPProperties->Sites.emplace_back(Point<>(x, y, z));
	VoroPProperties->num_vertices.emplace_back(pair<int, int>(i, p));//角点的个数
	output_vertices(i, x, y, z, VoroPProperties);//每个胞元的角点坐标
	VoroPProperties->num_faces.emplace_back(pair<int, int>(i, number_of_faces()));//存放voro单元面数

	//计算单个面片面积
	face_areas(vd);
	VoroPProperties->face_area.emplace_back(vd);//
	...
	double cx, cy, cz;
	centroid(cx, cy, cz);
	VoroPProperties->vorocellcenter.emplace_back(Point<>(x + cx, y + cy, z + cz));

	//Voronoi胞元体积
	VoroPProperties->Volume.emplace_back(volume());
