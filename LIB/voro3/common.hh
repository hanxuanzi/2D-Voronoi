...
//жиди
inline void voro_print_positions(std::vector<double>& v, VoroPProperties* VoroPProperties) {
	vector<Point<>>vorofacenormal;
	vorofacenormal.clear();
	if (v.size() > 0) {
		//fprintf(fp,"(%g,%g,%g)",v[0],v[1],v[2]);

		vorofacenormal.push_back(Point<>(v[0], v[1], v[2]));
		for (int k = 3; (unsigned int)k < v.size(); k += 3) {
			//fprintf(fp," (%g,%g,%g)",v[k],v[k+1],v[k+2]);
			vorofacenormal.push_back(Point<>(v[k], v[k + 1], v[k + 2]));
		}
	}
	VoroPProperties->face_normal.push_back(vorofacenormal);
}
...
void voro_print_face_vertices(int i, std::vector<int>& v, VoroPProperties* VoroPProperties);//жиди
