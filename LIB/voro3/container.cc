...
/** The class constructor sets up the geometry of container.设置容器几何体
 * \param[in] (ax_,bx_) the minimum and maximum x coordinates.
 * \param[in] (ay_,by_) the minimum and maximum y coordinates.
 * \param[in] (az_,bz_) the minimum and maximum z coordinates.设置最小最大xyz值
 * \param[in] (nx_,ny_,nz_) the number of grid blocks in each of the three
 *                       coordinate directions.三个坐标方向上的栅格块数
 * \param[in] (xperiodic_,yperiodic_,zperiodic_) flags setting whether the
 *                                               container is periodic in each
 *                                               coordinate direction.
 * \param[in] init_mem the initial memory allocation for each block. */
...
/** Computes all the Voronoi cells and saves customized information about them.计算所有维诺胞元
...
//重载
void container::print_custom(const char *format, VoroPProperties*VoroPProperties) {
	c_loop_all vl(*this);
	print_custom(vl, format, VoroPProperties);
}
